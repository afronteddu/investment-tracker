...
def _ask(prompt: str, max_tokens: int = 900) -> str:
    ...
    groq_key = os.getenv("MISTRAL_API_KEY", "")
    ...
    if groq_key:
        try:
            ...
            client = OpenAI(api_key=groq_key, base_url="https://api.mistral.ai/v1")
            ...
            if "rate_limit" not in err and "429" not in err:
                return f"Mistral error: {err[:200]}"
...
def ai_health_check() -> dict:
    ...
    if groq_key and not result["active"]:
        try:
            ...
            client = OpenAI(api_key=groq_key, base_url="https://api.mistral.ai/v1")
            resp = client.chat.completions.create(
                model="mistral-small-latest", max_tokens=5,
                messages=[{"role": "user", "content": "Reply OK"}],
            )
            if resp.choices[0].message.content:
                result["groq"] = True
                result["active"] = "mistral"
        except Exception:
            pass
